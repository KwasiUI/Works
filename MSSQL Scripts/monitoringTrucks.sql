with cte as (SELECT  ece.operator_name as 'Operator ID', eu.name as 'Operator',ece.che_name AS 'TRUCK', 
timestamp,type_description, 'WDAY'=cast(ece.timestamp as date)
  ...
  and (
		 ...
		 or (che_name like 'G2%') or (che_name like 'M0%')
			 ...
			 ,max(timestamp)OVER (PARTITION BY OPERATOR,TRUCK) 'LOG OFF'
			 from cte where   timestamp>='06-22-2022' 
